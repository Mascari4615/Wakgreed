...
public class RealWakgood : BossMonster
{
    ...
    private List<int> wordIndexes2;
    private List<int> wordLengths;
    private List<int> wordLengths2;
        ...
        if (textMesh.gameObject.activeSelf)
        {
            ...
            for (int w = 0; w < wordIndexes.Count; w++)
            {
                ...
                for (int i = 0; i < wordLengths[w]; i++)
                {
                    ...
                    colors[index + 1] =
                        rainbow.Evaluate(Mathf.Repeat(Time.time + (vertices[index + 1].x * 0.001f), 1f));
                    colors[index + 2] =
                        rainbow.Evaluate(Mathf.Repeat(Time.time + (vertices[index + 2].x * 0.001f), 1f));
                    colors[index + 3] =
                        rainbow.Evaluate(Mathf.Repeat(Time.time + (vertices[index + 3].x * 0.001f), 1f));
        ...
        if (textMesh2.gameObject.activeSelf)
        {
            ...
            for (int w = 0; w < wordIndexes2.Count; w++)
            {
                ...
                for (int i = 0; i < wordLengths2[w]; i++)
                {
                    ...
                    colors2[index + 1] =
                        rainbow2.Evaluate(Mathf.Repeat(Time.time + (vertices2[index + 1].x * 0.001f), 1f));
                    colors2[index + 2] =
                        rainbow2.Evaluate(Mathf.Repeat(Time.time + (vertices2[index + 2].x * 0.001f), 1f));
                    colors2[index + 3] =
                        rainbow2.Evaluate(Mathf.Repeat(Time.time + (vertices2[index + 3].x * 0.001f), 1f));
    ...
    protected override void OnEnable()
    {
        if (DropsCO != null)
        {
            StopCoroutine(DropsCO);
        }

        if (TheShipCO != null)
        {
            StopCoroutine(TheShipCO);
        }
    ...
    private IEnumerator Drops()
    {
        while (true)
        {
            Vector3 randomPos = Wakgood.Instance.transform.position +
                                ((Vector3)Random.insideUnitCircle * Random.Range(0, 20f));
    ...
    private IEnumerator TheShip()
    {
        ...
        {
            Rigidbody2D.velocity = (Wakgood.Instance.transform.position - transform.position).normalized *
        ...
        Vector3 rot = new(0, 0,
            (Mathf.Atan2(Wakgood.Instance.transform.position.y - (transform.position.y + 0.8f),
                Wakgood.Instance.transform.position.x - transform.position.x) * Mathf.Rad2Deg) - 90);
        for (int i = 0; i < 3; i++)
        {
            ObjectManager.Instance.PopObject("PanchiSlash",
                transform.position + (Vector3.up * 0.8f) + (direction * 1.5f), rot);
            yield return new WaitForSeconds(.3f);
        }
    ...
    private IEnumerator GTA()
    {
        ...
        {
            Vector3 pos1 = Wakgood.Instance.transform.position +
                           (((Vector3)Random.insideUnitCircle * 10f).normalized * 100);
            Vector3 pos2 = Wakgood.Instance.transform.position +
                           ((Wakgood.Instance.transform.position - pos1).normalized * 100);
            ...
            GameObject temp = Instantiate(icecream, pos1 + ((pos2 - pos1) / 2),
                Quaternion.Euler(new Vector3(0, 0, Mathf.Rad2Deg * Mathf.Atan2(pos2.y - pos1.y, pos2.x - pos1.x))));
            temp.transform.localScale =
                new Vector3(Vector3.Distance(pos1, pos2) * 0.25f, temp.transform.localScale.y, 1);
