...
public abstract class BossMonster : Monster
{
    ...
    protected override void Awake()
    {
        ...
        cinemachineTargetGroup = GameObject.Find("Cameras").transform.Find("CM TargetGroup")
    ...
    protected virtual IEnumerator _Collapse()
    {
        ...
        RuntimeManager.PlayOneShot(
            $"event:/SFX/Monster/{(name.Contains("(Clone)") ? name.Remove(name.IndexOf("(", StringComparison.Ordinal), 7) : name)}_Collapse",
            transform.position);
