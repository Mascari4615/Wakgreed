...
public class SuriswordPanchi : NormalMonster
{
        ...
        while (true)
        {
            ...
            while (Vector2.Distance(Wakgood.Instance.transform.position, transform.position) > 5f)
            {
                ...
                Rigidbody2D.velocity =
                    (Wakgood.Instance.transform.position - transform.position).normalized * MoveSpeed;
                yield return null;
            }
    ...
    private IEnumerator Attack()
    {
        ...
        SpriteRenderer.flipX = (transform.position + ((Vector3)Random.insideUnitCircle * 2f)).x <
                               Wakgood.Instance.transform.position.x;
        ...
        GameObject a = ObjectManager.Instance.PopObject("Suri", transform.position);
