...
public class WakgoodMove : MonoBehaviour
{
    ...
    private void Move()
    {
        ...
        {
            if (!hInputList.Contains(-1))
            {
                hInputList.Add(-1);
            }
        }
        else if (hInputList.Contains(-1))
        {
            hInputList.Remove(-1);
        }
        ...
        {
            if (!vInputList.Contains(1))
            {
                vInputList.Add(1);
            }
        }
        else if (vInputList.Contains(1))
        {
            vInputList.Remove(1);
        }
        ...
        if (playerRb.bodyType != RigidbodyType2D.Static)
        {
            PlayerRb.velocity = moveDirection *
                                (float)Math.Round(
                                    moveSpeed.RuntimeValue * (1 + ((float)moveSpeedBonus.RuntimeValue / 100)),
        ...
        StartCoroutine(TtmdaclExtension.ChangeWithDelay(!(mbCanBbolBbol = false), Random.Range(0.1f, 0.3f),
            value => mbCanBbolBbol = value));
    }
    ...
    private IEnumerator Dash()
    {
        ...
        {
            if (Physics2D.BoxCast(transform.position, new Vector2(.5f, .5f), 0f, moveDirection, 0.9f,
