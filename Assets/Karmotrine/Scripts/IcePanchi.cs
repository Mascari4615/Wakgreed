...
public class IcePanchi : NormalMonster
{
    ...
    private readonly IceMoongtange[] IceMoongtanges = new IceMoongtange[ICE_MOONGTANGE_COUNT];
    ...
    protected override void OnEnable()
    {
        ...
        bIsAttacking = false;
        base.OnEnable();
        foreach (IceMoongtange iceMoongtange in IceMoongtanges)
        {
    ...
    private IEnumerator SpawnIceMoongtange(int moongtangeIndex, Vector3 spawnPos)
    {
        ...
        for (int i = 0; i < ICE_PIECE_COUNT; i++)
        {
            ...
            IceMoongtanges[moongtangeIndex].ices[i].transform.localRotation =
                Quaternion.Euler(i * theta * Vector3.forward);
            IceMoongtanges[moongtangeIndex].ices[i].gameObject.SetActive(true);
            IceMoongtanges[moongtangeIndex].ices[i].SetDirection
            (new Vector3(Mathf.Cos((90 + (theta * i)) * Mathf.Deg2Rad),
                Mathf.Sin((90 + (theta * i)) * Mathf.Deg2Rad), 0));
    ...
    {
        foreach (IceMoongtange iceMoongtange in IceMoongtanges)
        foreach (BulletMove ice in iceMoongtange.ices)
        {
    ...
    {
        public readonly Collider2D[] iceColliders = new Collider2D[ICE_PIECE_COUNT];
        public readonly BulletMove[] ices = new BulletMove[ICE_PIECE_COUNT];
    }
}