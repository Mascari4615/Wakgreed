...
public class Dopamine : BossMonster
{
    ...
    private IEnumerator Monkey()
    {
        ...
        Vector3 targetPos = new(
            Mathf.Clamp(Wakgood.Instance.transform.position.x + ((-1 + (Random.Range(0, 2) * 2)) * 6),
                spawnedPos.x - moveLimit, spawnedPos.x + moveLimit),
            Mathf.Clamp(Wakgood.Instance.transform.position.y + ((-1 + (Random.Range(0, 2) * 2)) * 6),
        ...
        for (int i = 0; i < 3; i++)
        {
            attackDirection[i] = (Wakgood.Instance.transform.position - transform.position).normalized +
    ...
    private IEnumerator SpawnWakpago()
    {
        ...
        foreach (BulletMove monkey in monkeys)
        {
