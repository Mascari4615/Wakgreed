...
public class WakgoodCollider : MonoBehaviour
{
    ...
    {
        if (other.CompareTag("UpperDoor"))
        {
            GameManager.Instance.StartCoroutine(StageManager.Instance.MigrateRoom(Vector2.up, 1));
        }
        else if (other.CompareTag("LowerDoor"))
        {
            GameManager.Instance.StartCoroutine(StageManager.Instance.MigrateRoom(Vector2.down, 0));
        }
        else if (other.CompareTag("LeftDoor"))
        {
            GameManager.Instance.StartCoroutine(StageManager.Instance.MigrateRoom(Vector2.left, 3));
        }
        else if (other.CompareTag("RightDoor"))
        {
            GameManager.Instance.StartCoroutine(StageManager.Instance.MigrateRoom(Vector2.right, 2));
        }
        else if (other.CompareTag("AreaDoor"))
        {
            GameManager.Instance.ChangeArea(other.transform);
        }
    ...
    public InteractiveObject GetNearestInteractiveObject()
    {
        ...
        foreach (InteractiveObject item in nearInteractiveObjectDic.Values.Where(item =>
                     Vector2.Distance(transform.position, item.transform.position) < distance))
        {
