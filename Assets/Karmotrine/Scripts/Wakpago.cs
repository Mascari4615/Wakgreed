...
public class Wakpago : BossMonster
{
    ...
    private IEnumerator Bombs()
    {
        while (true)
        {
            Vector3 pos = (Vector3)Random.insideUnitCircle * moveLimit;
            Vector3 randomPos =
                new(Mathf.Clamp(spawnedPos.x + pos.x, spawnedPos.x - moveLimit, spawnedPos.x + moveLimit),
                    Mathf.Clamp(spawnedPos.y + pos.y, spawnedPos.y - moveLimit, spawnedPos.y + moveLimit));
    ...
    private IEnumerator Bullet()
    {
        ...
        for (float i = 0; i <= 1f; i += Time.deltaTime * 0.7f)
        {
    ...
    private IEnumerator Punch()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            Vector3 targetPos = new(
                Mathf.Clamp(Wakgood.Instance.transform.position.x + (-1 + (Random.Range(0, 2) * 2)),
                    spawnedPos.x - moveLimit, spawnedPos.x + moveLimit),
                Mathf.Clamp(Wakgood.Instance.transform.position.y + (-1 + (Random.Range(0, 2) * 2)),
