        ...
        while (true)
        {
            if (canWakggiddi)
            {
                if (GameManager.Instance.enemyRunTimeSet.Items.Count > 0)
                {
                    ...
                    {
                        temp += Time.deltaTime;
                        transform.SetPositionAndRotation(Vector3.Lerp(transform.position, mob.position, temp * 1.5f),
                            Quaternion.identity);
                        yield return null;
                    }
                    ...
                    while (Vector3.Distance(transform.position,
                               Wakgood.Instance.transform.position + originalDistance) > .5f)
                    {
                        temp += Time.deltaTime;
                        transform.position = Vector3.Lerp(transform.position,
                            Wakgood.Instance.transform.position + originalDistance, temp * 1.5f);
                        yield return null;
                    }
            ...
            position = Vector3.Lerp(position, wakgoodPosition + ((position - wakgoodPosition).normalized * 2),
