...
public class StageManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            {
                if (Input.GetKeyDown(KeyCode.Tab) && mapPanel.activeSelf == false)
                {
                    MapDoor(true);
                }
                else if (Input.GetKeyUp(KeyCode.Tab) && mapPanel.activeSelf)
                {
                    MapDoor(false);
                }
            }
            ...
            {
                if (Input.GetKeyDown(KeyCode.Tab) && shortCut.activeSelf == false)
                {
                    shortCut.SetActive(true);
                }
                else if (Input.GetKeyUp(KeyCode.Tab) && shortCut.activeSelf)
                {
                    shortCut.SetActive(false);
                }
            }
    ...
    public void GenerateStage()
    {
        ...
        /* 스테이지 틀 만들기 */
        {
            ...
            while (roomMolds.Count < _roomCount)
            {
                ...
                if (roomMolds.Find(x => x.Coordinate == totalRoomMoldCoordinate) != null)
                {
                    continue;
                }
        ...
        for (int i = 0; i < mapGridLayoutGroup.transform.childCount; i++)
        {
            if (i <= (stageEdgeLength * stageEdgeLength) - 1)
            {
                ...
                if (roomDic.ContainsKey(targetRoomCoordinate))
                {
                    ...
                    temp.GetChild(0).GetComponent<Image>().color =
                        new Color(189 / 255f, 81 / 255f, 90 / 255f); // Property\CurrentRoom
                    ...
                    {
                        temp.GetChild(2).gameObject
                            .SetActive((targetRoom as ShopRoom).IsItemShop); // Property\Icon\Item
                        temp.GetChild(3).gameObject
                            .SetActive(!(targetRoom as ShopRoom).IsItemShop); // Property\Icon\Food
                    }
                    else
                    {
    ...
    {
        scrollRectBackGround.localPosition =
            -CurrentRoom.Coordinate * (mapGridLayoutGroup.cellSize.x + mapGridLayoutGroup.spacing.x);
        ...
        roomUiDic[CurrentRoom.Coordinate].GetChild(1).GetChild(0).GetComponent<Image>().color =
            new Color(0f / 255f, 200f / 255f, 255f / 255f); // Property\CurrentRoom
    }

    public void SeeAllMap()
    {
        foreach (KeyValuePair<Vector2, Transform> item in roomUiDic)
        {
            Transform t = item.Value;
    ...
    public IEnumerator MigrateRoom(Vector2 moveDirection, int spawnDirection)
    {
        ...
        if (roomUiDic.ContainsKey(CurrentRoom.Coordinate) == false ||
            roomUiDic.ContainsKey(CurrentRoom.Coordinate + moveDirection) == false)
        {
            if (roomUiDic.ContainsKey(CurrentRoom.Coordinate) == false)
            {
        ...
        roomUiDic[CurrentRoom.Coordinate].GetChild(1).GetChild(0).GetComponent<Image>().color =
            new Color(200f / 255f, 200f / 255f, 200f / 255f); // Property\CurrentRoom
        ...
        UIManager.Instance.SetRoomName(CurrentRoom);

        Wakgood.Instance.transform.position =
            CurrentRoom.Doors[spawnDirection].transform.position + ((Vector3)moveDirection * 4);
    ...
    public IEnumerator MigrateRoom(Vector2 coordinate)
    {
        ...
        Wakgood.Instance.transform.position = CurrentRoom.CenterSpawnPoint != null
            ? CurrentRoom.CenterSpawnPoint.position
    ...
    private void MapDoor(bool bOpen)
    {
        ...
        {
            scrollRectBackGround.localPosition = -CurrentRoom.Coordinate *
                                                 (mapGridLayoutGroup.cellSize.x + mapGridLayoutGroup.spacing.x);
    ...
    {
        if (CurrentRoom is NormalRoom room)
        {
            room.CheckMonsterCount();
        }
    }
