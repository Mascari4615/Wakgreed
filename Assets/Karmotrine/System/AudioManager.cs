...
public class AudioManager : MonoBehaviour
{
    ...
    private readonly string[] bgmTitles = { "yeppSun - 버거워", "yeppSun - 세구의 꿈", "yeppSun - 장난기 기능" };
    private readonly string[] bossBgmTitles = { "Badassgatsby - 왁스라다", "Badassgatsby  - 왁그란 투리스모", "추르르 - Wakgood FC" };
    private int i;
    ...
    private void Update()
    {
        ...
        else if (isBossing.RuntimeValue)
        {
            ...
            if (pbState == PLAYBACK_STATE.STOPPED)
            {
                ...
                BgmEvent = RuntimeManager.CreateInstance(
                    $"event:/BGM/{bossBgmTitles[StageManager.Instance.currentStageID]}");
                BgmEvent.start();
            }
        }
    ...
    public void PlayMusic(string musicName)
    {
        ...
        if (UIManager.Instance != null)
        {
            UIManager.Instance.SetMusicName(musicName);
        }
