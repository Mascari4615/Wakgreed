...
public class Weapon : HasGrade, ISerializationCallbackReceiver
{
    ...
    [NonSerialized] private bool canUseBaseAttack = true;
    [NonSerialized] private bool canUseSkillE = true;
    [NonSerialized] private bool canUseSkillQ = true;
    [NonSerialized] private bool canUseSpecialAttack = true;
    [NonSerialized] private float curBaseAttackCoolTime;
    [NonSerialized] public float CurReloadTime;
    [NonSerialized] public float CurSkillECoolTime;
    [NonSerialized] public float CurSkillQCoolTime;
    [NonSerialized] public float CurSpecialAttackCoolTime;
    [NonSerialized] public bool IsReloading;
    [NonSerialized] private IEnumerator reload;
    ...
    {
        if (!canUseBaseAttack || IsReloading)
        {
            return;
        }
        ...
        if (baseAttack.type.Equals(SkillType.Base))
        {
            if (DataManager.Instance.wgItemInven.Items.Contains(DataManager.Instance.ItemDic[53]))
            {
                ...
                {
                    ObjectManager.Instance.PopObject("Ball", Wakgood.Instance.transform.position)
                        .GetComponent<BulletMove>().SetDirection((Vector3)Wakgood.Instance.worldMousePoint -
    ...
    {
        CurReloadTime = 0;
        while ((CurReloadTime += Time.deltaTime) < reloadTime * (1 - (Wakgood.Instance.reloadSpeed.RuntimeValue / 100)))
        {
            yield return null;
        }
    ...
    {
        if (!canUseSpecialAttack || !specialAttack)
        {
            return;
        }
    ...
    public override void OnRemove()
    {
        ...
        {
            GameManager.Instance.StopCoroutine(reload);
        }
